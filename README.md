...
- La photo d'un repas analysé s'affiche **annotée dans sa fiche** comme à l'évaluation : contours colorés selon la confiance, pastilles numérotées tappables (nom, glucides estimés, niveau de confiance). Pour une photo enregistrée **avant** cette fonction (donc sans géométrie), un bouton **« ✨ Annoter la photo »** fait resituer les aliments déjà connus du repas par l'IA — elle ne réévalue rien, elle pointe — et la fiche devient annotée pour de bon.
...
La version courante est **v2.41.0**, affichée dans le pied de page de l'app (`#app-version` dans `index.html`).
